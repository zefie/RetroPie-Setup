...
rp_module_repo="git https://github.com/taleden/RetroPie-Manager-Plus.git retropie"
rp_module_section="exp"
rp_module_flags="noinstclean nobin"

function depends_retropie-manager() {
    local depends=(python2-dev python2-pip-whl python2-setuptools-whl virtualenv)
    getDepends "${depends[@]}"
}
...
function install_retropie-manager() {
    cd "$md_inst"
    git config --global --add safe.directory /opt/retropie/supplementary/retropie-manager
    sed -i 's| --no-site-packages||' "${md_inst}/Makefile"
